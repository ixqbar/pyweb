...
##Step 0 初始化
```
{node}/server_list/s1   {"create_time":x,"server_name":x,"server_id":x}
...
{node}/server_list/sN   {"create_time":x,"server_name":x,"server_id":x}
```

##Step 1 打包
```
{node}/to_zip_notice/v1  {"create_time":x,"pub_id":x,"pub_node_id":x,"config_version":x,"game_version":x,"status":"ok","finish_time":x}
{node}/to_zip_result/v1  {"create_time":x,"status":"ok"}
```

##Step 2 同步
```
{node}/to_syc_notice/v1    {"create_time":x,"pub_id":x,"pub_node_id":x,"config_version":x,"game_version":x,"servers":[x,x,...],"status":"ok","finish_time":x}
...
##Step 3 发布
```
{node}/to_pub_notice/v1    {"create_time":x,"pub_id":x,"pub_node_id":x,"config_version":x,"game_version":x,"servers":[x,x,...],"status":"ok","finish_time":x}
