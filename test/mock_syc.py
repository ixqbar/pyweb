...
class mock_syc(object):
    ...
    def syc_execute(self, config_version, game_version, server_id):
        ...
        LOG.info('start to execute shell %s/syc.sh %s %s %s' % (self._shell_path, config_version, game_version, server_id, ))
        result = subprocess.call('%s/syc.sh %s %s %s > /dev/null 2>&1' % (self._shell_path, config_version, game_version, server_id, ), shell = True)
