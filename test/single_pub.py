...
class mock_pub(object):
    ...
    def pub_execute(self, config_version, game_version):
        ...
        LOG.info('start to execute shell to pub for server_id=%s config_version=%s game_version=%s' % (self._server_id, config_version, game_version, ))
        result = subprocess.call(os.path.join(os.getcwd(), 'pub.sh %s %s %s > /dev/null 2>&1' % (config_version, game_version, self._server_id, )), shell = True)
